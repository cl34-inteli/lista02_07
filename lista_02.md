...
xxXB)Xxx O código avalia a expressão booleana, imprime o resultado `true`, calcula a soma dos números de 1 a 5 e imprime o resultado no console.
...
XXB)XX A saída de calcularOrcamento() seria: 'Seu saldo é negativo de -1050.' e a de calcularOrcamento2() seria: 'Seu saldo é negativo de -100.'
...
XXXD)XXX O código verifica se o número é par, se é divisível por 3 ou se é ímpar. Se for par, exibe a mensagem "O número é par!". Se for divisível por 3 (e não for par), exibe a mensagem "O número é divisível por 3!". Se for ímpar (e não for divisível por 3), exibe a mensagem "O número é ímpar e não é divisível por 3!".
...
XXXB)XXX Preload -> Create -> Update
...
XXXB)XXX Simular interações físicas realistas, como colisões e movimentos, em jogos 2D.
...
receba valor_idade
leia valor_idade
se valor_idade for menor que 16
    imprima "Não pode votar!"
senão se valor_idade for maior ou igual a 18
    imprima "Voto obrigatório!"
senão imprima "Voto facultativo!"
...
Classe Retangulo herda de FormaGeometrica:
    Método construtor(cor, base, altura)
        Atributos da classe pai: cor
        Atributos adicionais: base, altura
    Método calcularArea()
        RETORNE base * altura

Classe Circulo herda de FormaGeometrica:
    Método construtor(cor, raio)
        Atributos da classe mãe: cor
        Atributos adicionais: raio
    Método calcularArea()
...
enquanto (distancia_atual < distancia_final) e (tempo < tempo_maximo) e (velocidade_atual < velocidade_maxima) faca
    tempo <- tempo + 1
    velocidade_atual <- velocidade_inicial + aceleracao * tempo
    se velocidade_atual > velocidade_maxima entao
        velocidade_atual <- velocidade_maxima
    fim se
    distancia_atual <- (velocidade_inicial * tempo) + (0.5 * aceleracao * tempo^2)
fim enquanto

se distancia_atual >= distancia_final entao
    imprima "Uau, o carro conseguiu correr em ", tempo, " minutos!"
senao
    imprima "Ops, o carro não conseguiu terminar a corrida a tempo. Que pena!"
fim se
```
______
...
Função  MultiplicaçãoDeMatrizes(matrizA, matrizB):
    ...
    Senão:
        linhasA <- tamanho altura(matrizA)
        colunasA <- tamanho largura(matrizA)
        colunasB <- tamanho largura(matrizB)
        matrizResultado <- novaMatriz(linhaA, colunasB)
